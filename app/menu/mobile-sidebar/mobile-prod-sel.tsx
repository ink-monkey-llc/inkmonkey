...
function MobileProdSel() {
  ...
  //  ]
  const currentOption = slug
    ? productTypes.find((type) => type.url.includes(slug[0]))
    : null;
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'bg-bg-tertiary rounded-md cursor-pointer absolute z-20 w-full border-2 border-accent-tr ',
      ...
      >
        {productTypes.map((type, i) => (
          ...
          >
            ...
            >
              <h2 className=" text-txt-primary  font-bold text-lg group-hover:text-accent-bright">
                {type.label}
              </h2>
            </div>
          </Link>
