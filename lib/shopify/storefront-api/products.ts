...
const productApi = {
  getAllProducts: async (args: {
    ...
    const { data, errors, extensions } = await client.request(
      args.dir === 'prev' ? previousAllProductsQuery : nextAllProductsQuery,
      {
        variables,
  ...
  getProductsByType: async (args: {
    ...
    const { data, errors, extensions } = await client.request(
      args.dir === 'prev' ? previousProductByTypeQuery : nextProductByTypeQuery,
      {
        variables,
        apiVersion: API_VERSION,
      }
  ...
  searchProducts: async (args: {
    ...
    const { data, errors, extensions } = await client.request(
      args.dir === 'prev' ? searchPrevQuery : searchNextQuery,
      {
        variables,
        apiVersion: API_VERSION,
      }
  ...
  searchAllProducts: async (args: {
    ...
    const { data, errors, extensions } = await client.request(
      args.dir === 'prev' ? searchAllPrevQuery : searchAllQuery,
      {
        variables,
        apiVersion: API_VERSION,
      }
  ...
  getProductHandles: async (args: {
    after: string;
    first: number;
    isFirst: boolean;
  }) => {
    const { data, errors, extensions } = await client.request(
      args.isFirst ? getFirstProductHandlesQuery : getProductHandlesQuery,
      {
        variables: {
          after: args.after,
          first: args.first,
